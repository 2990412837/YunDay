...
public class UserService {
     ...
     * 修改用户信息
     1. 获取参数（昵称、心情）
     2. 参数的非空校验（判断必填参数非空）     如果昵称为空，将状态码和错误信息设置resultInfo对象中，返回resultInfo对象
    ...
    public ResultInfo<User> updateUser(HttpServletRequest request) {
        ...
        try {
            ...
            // 2. 通过Part对象获取上传文件的文件名(从头部信息中获取上传文件的文件名)
            ...
            if (!StrUtil.isBlank(fileName)) {
                ...
                // 4. 获取文件存放的路径 WEB-INF/upload/目录中
                String filePath = request.getServletContext().getRealPath("/WEBINF/upload/");
